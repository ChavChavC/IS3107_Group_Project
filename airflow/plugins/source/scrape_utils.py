...
def booking_scraper():
    ...
    raw_booking_filepath = '/opt/airflow/dags/data/booking/booking_data.csv'
    ...
    def main():
        with open('/opt/airflow/dags/data/booking/USA.xml', 'r') as f:
            data = f.read()
        ...
        data_frame = pd.DataFrame(columns=['hotel_name', 'description', 'popular_facilities', 'overall_rating', 'total_num_of_reviews', 'region', 'region_shortform', 'cleanliness', 'service', 'location', 'facilities', 'value'])

        hotel_data = process_hotels(hotels_list)
        for hotel in hotel_data:
            data_frame.loc[len(data_frame)] = hotel
        data_frame = data_frame.dropna()
        data_frame.to_csv(raw_booking_filepath, index=None)
